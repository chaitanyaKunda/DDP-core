...
public class DdpAedDailySLASchedulerTask implements Task{

	private static final Logger logger = LoggerFactory.getLogger(DdpAedDailySLASchedulerTask.class);
	...
	public void execute(String jobName) {
		...
		logger.info("Inside the DdpAedDailySLASchedulerTask.execute(String jobName) & Job Name : "+jobName);
		
		// Need to fetch details from the ddp rule details base daily selection.
		List<DdpRuleDetail> ruleDetails = commonUtil.fetchRuleDetailsBasedSLAFrequency(Constant.DDP_SQL_SELECT_AED_DAILY_SCH);
		...
		if (ruleDetails != null && ruleDetails.size() > 0) {
			...
			for (String key :  ddpRuleMap.keySet()) {
				...
				try {
						...
						logger.error("DdpAedDailySLASchedulerTask.execute(String jobName) - Unable to send details to customer for rule detail id : "+ddpRuleDetail.getRdtId(), ex.getMessage());
						taskUtil.sendMailByDevelopers(ex.getMessage(), " inside  DdpAedDailySLASchedulerTask.execute(String jobName) for Rule Detail ID : "+ddpRuleDetail.getRdtId());
					}
				
			}
		}
		
		logger.info("End of DdpAedDailySLASchedulerTask.execute(String jobName) & Job Name : "+jobName);
	}
