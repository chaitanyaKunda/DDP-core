...
public class DdpAedWeeklySLASchedulerTask implements Task {

	private static final Logger logger = LoggerFactory.getLogger(DdpAedWeeklySLASchedulerTask.class);
	...
	public void execute(String jobName) {
		...
		logger.info("Inside the DdpAedWeeklySLASchedulerTask.execute(String jobName) & Job Name : "+jobName);
		
		// Need to fetch details from the ddp rule details base weekly selection.
		List<DdpRuleDetail> ruleDetails = commonUtil.fetchRuleDetailsBasedSLAFrequency(Constant.DDP_SQL_SELECT_AED_WEEKLY_SCH);
		...
		if (ruleDetails != null && ruleDetails.size() > 0) {
				...
				for (String key :  ddpRuleMap.keySet()) {
					...
					try {
						...
						logger.error("DdpAedWeeklySLASchedulerTask.execute(String jobName) - Unable to send details to customer for rule detail id : "+ddpRuleDetail.getRdtId(), ex.getMessage());
						taskUtil.sendMailByDevelopers(ex.getMessage(), " inside  DdpAedWeeklySLASchedulerTask.execute(String jobName) for Rule Detail ID : "+ddpRuleDetail.getRdtId());
					}
				
			}
		}
		
		logger.info("End of DdpAedWeeklySLASchedulerTask.execute(String jobName) & Job Name : "+jobName);
	}
